...
const YesterdayPage: React.FC = () => {
  ...
  const ordersReceivedYesterday: Order[] = [
    {
      id: '1',
  ...
  const ordersDueYesterday: Order[] = [
    {
      id: '1',
  ...
  const completedTasks = tasksYesterday.filter((t) => t.status === 'completed').length
  const totalOrders = ordersReceivedYesterday.length + ordersDueYesterday.length
    ...
    <div className="p-6 bg-secondary min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <DataCard
          ...
          value={tasksYesterday.length}
          icon={CheckCircle2}
          description={`${completedTasks} completed`}
        />
        <DataCard
          header="Orders Received"
          value={ordersReceivedYesterday.length}
          icon={ShoppingBag}
          description={`Total: $${ordersReceivedYesterday.reduce((sum, o) => sum + o.amount, 0).toLocaleString()}`}
        />
        <DataCard
          header="Orders Due"
          value={ordersDueYesterday.length}
          icon={TrendingUp}
          description={`Total: $${ordersDueYesterday.reduce((sum, o) => sum + o.amount, 0).toLocaleString()}`}
        />
        <DataCard
          header="Total Orders"
          value={totalOrders}
          icon={ShoppingBag}
          description={`$${(ordersReceivedYesterday.reduce((sum, o) => sum + o.amount, 0) + ordersDueYesterday.reduce((sum, o) => sum + o.amount, 0)).toLocaleString()} value`}
        />
      </div>
        ...
        <div>
          <div className=" rounded-lg  py-6">
            ...
            <div className="space-y-2 max-h-[400px] overflow-y-auto">
              {orderTab === 'due'
                ? ordersDueYesterday.length === 0
                  ? (
                    <div className="text-center py-8">
                      <p className="text-zinc-500">No orders due Yesterday</p>
                    </div>
                    )
                  : (
                    ordersDueYesterday.map((order, index) => (
                      <div
                        key={order.id}
                ...
                : ordersReceivedYesterday.length === 0
                  ? (
                    <div className="text-center py-8">
                      <p className="text-zinc-500">No orders received Yesterday</p>
                    </div>
                    )
                  : (
                    ordersReceivedYesterday.map((order, index) => (
                      <div
                        key={order.id}
